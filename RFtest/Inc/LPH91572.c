...
void LCD_init(void)
{
 ...
 #ifdef _8_BIT_COLOR
 Send_to_lcd(DAT, 0x02); //Байт на пиксель 256 цветов  0x2
 #else
...
void LCD_Puts(char *str,int length int x, int y,  int t_color, int b_color, char zoom_width, char zoom_height, int rot)
{
 ...
 for(i=0;i<length;i++) //x и y - адрес пикселя начальной позиции; с увеличением переменной i адрес вывода очередного символа смещается на i*6 (чем организуются столбцы дисплея)  
 {      
